...
import { agentesCache } from "@/lib/agentes-cache";
...
export function useAgentes() {
  ...
  const { triggerRefresh } = useAgent();

  const invalidateCache = useCallback(() => {
    setState(prev => ({ ...prev, refreshKey: prev.refreshKey + 1 }));
    agentesCache.invalidate(); // Invalidar cache compartilhado
    triggerRefresh(); // Disparar atualização global
  }, [triggerRefresh]);
  ...
  const getAgentes = useCallback(async () => {
    try {
      return await agentesCache.getAgentes();
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : "Erro ao buscar agentes em deploy";
      toast.error(errorMessage);
      throw error;
    }
  }, []);

  const getCustomizedAgentes = useCallback(async () => {
    try {
      const response = await fetch("/api/agentes/customized");

      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.error || "Erro ao buscar agentes customizados");
      }
      ...
      const errorMessage = error instanceof Error ? error.message : "Erro ao buscar agentes customizados";
      toast.error(errorMessage);
      throw error;
    }
  }, []);
