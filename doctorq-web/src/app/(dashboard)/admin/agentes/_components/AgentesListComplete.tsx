...
import { AgenteFormComplete } from "./AgenteFormComplete";
import type { Agente } from "@/types/agentes";

export function AgentesListComplete() {
  const router = useRouter();
  const [showForm, setShowForm] = useState(false);
  const [showDeleteDialog, setShowDeleteDialog] = useState(false);
  const [agenteToDelete, setAgenteToDelete] = useState<Agente | null>(null);
  const [selectedAgente, setSelectedAgente] = useState<Agente | undefined>();
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      await fetchAgentes(1, "", "dt_criacao", true, 10);
      setIsInitialLoad(false);
    };

    loadInitialData();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []); // fetchAgentes is memoized, only run on mount

  useEffect(() => {
    if (isInitialLoad) return;

    const timer = setTimeout(() => {
      fetchAgentes(1, searchTerm, "dt_criacao", true, pageSize);
      setCurrentPage(1);
    }, 500);
    return () => clearTimeout(timer);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchTerm]); // fetchAgentes is memoized, pageSize is captured
  ...
  useEffect(() => {
    ...
    setCurrentPage(1);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pageSize]); // fetchAgentes is memoized, searchTerm is captured
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <Card className={`border-0 shadow-lg transition-opacity duration-200 ${!isInitialLoad && loading ? 'opacity-70' : 'opacity-100'}`}>
          <Table>
            <TableHeader>
              <TableRow>
              ...
              {isInitialLoad && loading ? (
                ...
                agentes.map((agente) => (
                  <TableRow key={agente.id_agente}>
                    ...
                    <TableCell className="text-sm text-muted-foreground text-center" suppressHydrationWarning>
                      {formatDate(agente.dt_atualizacao || agente.dt_criacao)}
                    </TableCell>
