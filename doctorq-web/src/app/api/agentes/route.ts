...
export async function GET(request: NextRequest) {
  try {
    ...
    // Header especial para obter apenas o agente principal
    const stPrincipal = request.headers.get("st-principal");
    if (stPrincipal?.toLowerCase() === "true") {
      const response = await aiServiceFetch("/agentes/", {
        method: "GET",
        headers: {
          "st-principal": "true",
        },
    ...
    const response = await aiServiceFetch(`/agentes/?${searchParams.toString()}`);
    if (!response.ok) {
      return buildErrorResponse(response);
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!nm_agente || typeof nm_agente !== "string" || !nm_agente.trim()) {
      return jsonResponse({ error: "nm_agente is required" }, 400);
    }
