...
/**
 * API Route para remarcar agendamento pelo paciente
 */
  ...
  try {
    ...
    if (!body.nova_data_hora) {
      return NextResponse.json(
        { error: 'Nova data e hora são obrigatórios' },
        { status: 400 }
      );
    }
    ...
    // Agora fazer a remarcação com o id_paciente correto
    const response = await fetch(`${backendUrl}/agendamentos/${id}/remarcar-paciente`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        motivo: body.motivo || 'Remarcado pelo paciente',
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: data.detail || data.error || 'Erro ao remarcar agendamento' },
