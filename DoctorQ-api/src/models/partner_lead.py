...
class PartnerLeadServiceResponse(BaseModel):
    ...
    @classmethod
    def extract_from_service_definition(cls, data: Any) -> Any:
        """Extrair service_code e price_label do relacionamento service_definition"""
        if hasattr(data, 'service_definition') and data.service_definition:
            # Extrair service_code do service_definition
            if not hasattr(data, 'service_code') or not data.service_code:
                data.service_code = data.service_definition.service_code

            # Usar price_label_snapshot se existir, senão usar do service_definition
            if hasattr(data, 'price_label_snapshot'):
                data.price_label = data.price_label_snapshot or (
                    data.service_definition.price_label if hasattr(data.service_definition, 'price_label') else None
                )
            elif not hasattr(data, 'price_label') or not data.price_label:
                data.price_label = data.service_definition.price_label if hasattr(data.service_definition, 'price_label') else None
...
class PartnerLeadResponse(BaseModel):
    ...
    created_at: datetime
    updated_at: datetime
    services_selected: List[PartnerLeadServiceResponse] = Field(alias="services")
