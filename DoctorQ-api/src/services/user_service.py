...
class UserService:
    ...
    async def create_user(self, user_data: UserCreate) -> User:
        ...
        try:
            ...
            db_user = User(
                ...
                st_ativo=user_data.st_ativo if user_data.st_ativo is not None else 'S',
    ...
    async def login_local(self, login_data: UserLoginLocal) -> str:
        ...
        # Garantir que nr_total_logins seja inteiro antes de incrementar
        current_logins = int(user.nr_total_logins) if user.nr_total_logins else 0
        user.nr_total_logins = current_logins + 1
        await self.db.commit()
    ...
    async def oauth_login(self, oauth_data: "UserOAuthLogin") -> Tuple[User, str]:
        ...
        if user:
            ...
            user.dt_ultimo_login = datetime.now()
            user.nr_total_logins = (user.nr_total_logins or 0) + 1
