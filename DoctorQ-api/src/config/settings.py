...
class AppSettings(BaseSettings):
    ...
    def allowed_cors_origins(self) -> List[str]:
        ...
        # Adicionar URL permitida principal
        if self.url_permitida:
            origins.add(self.url_permitida)

        # Adicionar variações de localhost e rede local
        if "localhost" in (self.url_permitida or ""):
            # Se localhost está permitido, adicionar também o IP da rede
            origins.add("http://10.11.2.81:3000")
        elif "10.11.2.81" in (self.url_permitida or ""):
            # Se IP está permitido, adicionar também localhost
            origins.add("http://localhost:3000")

        # Adicionar origins configuradas via CORS_ORIGINS
